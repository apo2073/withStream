...
object Managers {
    ...
    fun showTitle(mainTitle: String, subTitle: String, player: Player) {
        try {
            player.showTitle(Title.title(Component.text(mainTitle), Component.text(subTitle), Title.Times.times(Duration.ZERO, Duration.ofSeconds(5), Duration.ZERO)))
            ...
            val sendPacket = connection.javaClass.getMethod("sendPacket", Class.forName(NMSVersion.getPacketClass()))
